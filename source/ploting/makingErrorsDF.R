...
##########################################################################################################
##########################################################################################################
data = dget("NoTbars-CV-all-predictions.RData")
ret = sumError(data)
dump("ret", file="No-Tbars-Sum-Error.RData")


data2 = dget("Tbars-CV-all-predictions.RData")
ret2 = sumError(data2)
dump("ret2", file="Tbars-Sum-Error.RData")

# - tenho 30 execucoes individuais em uma lista
# quero contar a quantidade de cada erro para cada algoritmo
...
improvement.plot = function(ret){

  # full = do.call("rbind", ret)
  full = do.call("rbind", ret2)
  ...
  setEPS();
  filename = paste("algorithms-errors.eps", sep="");
  postscript("algorithms-errors.eps") #, height=10, width=6);
   g = NULL
